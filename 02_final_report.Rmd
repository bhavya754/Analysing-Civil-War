...
subtitle: "A replication and extension of Greed and Grievance in Civil War by Paul Collier and Anke Hoeffler, 2000"
...
The following code reads the data, and creates helper functions to summarize tables, and convert arrays to dataframes.
...
The first two extensions involve testing the proposed models. In the paper, though the authors show the results of the logistic regression, they do not provide any evidence in the form of in-sample or out-of-sample testing to back their claims regarding the utility of these models. We perform both in-sample and out-of-sample testing using k-fold cross validation and present our findings here.
...
The accuracy is high for the last two opportunity models while their sensitivity is low because the last two models contain diasporas which result in the loss of lots of data-points (at threshold 0.1). The sensitivity is very low and the accuracy is very high for all the models at threshold 0.5 as expected.
...
The in-sample results of the grievance models are comparable to those of the opportunity model at threshold 0.1. In model 3 we see that accuracy, sensitivity, and specificity all increase. We notice that this model uses land inequality Gini Index as a feature instead of income iniequality. This may mean that land inequality is important.
...
We see that model 7 performs the best out of all combined models. The results are comparable to the opportunity model (even though the best opportunity model has a higher accuracy, this is because of the inclusion of diasporas as a feature). 

The features used in the best combined model are:

Primary export commodity
Male secondary school enrollment
GDP growth
Number of year of peace preceding the war
...
print("Averaged K-fold validation results (for threshold=0.1)")
print(result)
...
names(result)[1]<-"model"
print("Averaged K-fold validation results (for threshold=0.5)")
print(result)

```

As we saw in the in-sample testing, combined model 7 performs best overall. Even though the best opportunity model has a slightly higher accuracy, its specificity is low.


## 4.3 Regularized Model

A regularised model is trained on all features except the diaspora related features and the accuracy achieved is comparable to the previous models. The sensitivity decreases drastically for threshold = 0.5, even though accuracy increases. The diaspora related features are removed because including them results in the loss of a majority of the data-points and the resulting regression is not really viable. 
...
for(testIndex in 1:k){
  ...
  regularized.data <- shuffled_data %>% select(warsa, coldwar, prevwara, peace, elfo, rf, frac, geogia, mount, lnpop, sxp, sxp2, dem, ygini, lgini, lngdp_, gy1, secm, pol16, oilsxp, oilsxp2, etdo4590)
  ...
  #plot(regularized_fit)
  tmp_coeffs <- coef(regularized_fit, s = "lambda.min")
  words <- data.frame(name = tmp_coeffs@Dimnames[[1]][tmp_coeffs@i +
1], coefficient = tmp_coeffs@x)
  words <- arrange(words, desc(coefficient))
  feats <- rbind(feats,words)
...
t <- feats %>% group_by(name) %>% summarise(n = mean(coefficient))
...
## 4.4 Summary of our work

1) We replicated the 5 opportunity, 3 grievance and 7 combined models with their corresponding subset of features proposed in the paper.

2) We performed in-sample and out of sample testing (using k-fold cross validation technique) on each of these models at different threshold values (to account for the heavy imbalance in the dataset).

3) A regularised GLM model was trained on the raw data with all the features and tested using k-fold cross validation.

## 4.5 Observations 

1) The dataset is heavily skewed (78 1's (civil war occurred and 1089 0's (civil war did not occur)). Moreover, we lose lots of data-points because we need to ignore a data-point if atleast one attribute in it has a missing value (we lost 121 data-points). 

2) The accuracy and specificity are higher, while the sensitivity is very low for models at threshold 0.5 (because the model almost always predicts 0). The accuracy and specificity decrease but the sensitivity increase dramatically at threshold 0.1. So the number of civil wars we’re actually predicting at this threshold is very low, and while the accuracy and specificity increase (because the dataset is skewed), this does not mean that this model performs better. 

3) Broadly speaking, the performance of the first three opportunity models is comparable to the grievance models. However, the last two opportunity models tend to outperform even the combined models (in terms of accuracy). This could also be chalked out to the fact that the number of observations become very low for the last two opportunity models because the inclusion of diasporas reduces the number of available observations. 

4) The performance of the regularized GLM is not necessarily better than the proposed models. However, this model gives us a great idea about the actual utility of each of the features. From the weights of the features which are displayed in our report, here are a few key observations regarding the features:

i) The features with significant explanatory power with positive correlation are Primary Commodity Export, Square of the dummy variable for oil export, Ethnic dominance, Population size and whether civil war occurred in the past or not, in that order. 

ii) The features with significant explanatory power with negative correlation are square of the primary commodity export, polarization in society, dummy variable for oil export, geographic dispersion, whether the country was involved in the cold war or not and the GDP of the country, again in that order.

iii) The features which do not have significant explanatory power are: Democracy index, linguistic diversity, social fractionalisation, GDP growth, Land inequality, Mountainous Terrain, time since the last civil war, Religious fractionalisation, male secondary enrollment, Income inequality. 

iv) According to our analysis, the feature "polarization in society" has good explanatory power. However, this is a feature which comes under the grievance model. According to the authors, the only feature from the grievance model which has significant explanatory power is ethnic dominance (ethnic dominance is highly positively correlated according to our analysis too). Therefore, our analysis seems to contradict the assertion made by the authors in this regard. The descriptive data does not suggest that polarization is important: conflict and peace episodes have very similar mean values (Table 2). Moreover, during the replication process of the combined model, we find that in that particular model the feature is not significant, and thus we dismissed it. 

5) Our findings seem to generally conform to the authors' claim that "greed is a better indicator than grievance for predicting civil wars”. 
However, grievance cannot be dismissed completely as is shown by the combined models. Furthermore, another feature in the grievance model that is important is polarization in society according to our ananlysis. 
